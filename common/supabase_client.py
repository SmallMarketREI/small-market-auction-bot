...
def delete_eq(table: str, column: str, value) -> None:
    """Delete every row in `table` where `column` == `value`. Used to remove
    one specific row (e.g. a watch_auctions row that just graduated to
    Sold) -- unlike delete_not_in, this only ever touches the exact row
    named, never a whole set defined by exclusion."""
    config.require_supabase_config()
    url = f"{config.SUPABASE_URL}/rest/v1/{table}"
    resp = requests.delete(
        url,
    ...
    if resp.status_code not in (200, 204):
        raise RuntimeError(f"Supabase delete_eq on {table} failed: {resp.status_code}: {resp.text[:500]}")
